...
fc_net = ((128, True), )
hidden_layers = (256, 512)  #(256, 512, 1024)
noise_dim = 256

alf.config(
    'create_dataset',
    dataset_name='cifar10',
    train_batch_size=50,  #train_batch_size,
    test_batch_size=100)
...
alf.config(
    ...
    entropy_regularization=1e-5,  # .5,
    force_fullrank=False,
    fullrank_diag_weight=1.0,  #0.003,
    block_pinverse=False,
    jac_autograd=False,
    pinverse_hidden_size=256,  #512,
    optimizer=alf.optimizers.Adam(
        lr=1e-5, weight_decay=1e-4),  #lr=1e-4, weight_decay=1e-4),
